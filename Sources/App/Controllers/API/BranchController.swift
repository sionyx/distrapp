...
struct BranchController {
    ...
    func upload(_ req: Request) throws -> EventLoopFuture<Response> {
        ...
        // create dir
        let dirPath = URL(fileURLWithPath: "./builds/\(params.project)/\(params.branch)")
        print("dir path: \(dirPath.absoluteString)")
        try FileManager.default.createDirectory(atPath: dirPath.path, withIntermediateDirectories: true, attributes: nil)

        let filePath = URL(fileURLWithPath: "./builds/\(params.project)/\(params.branch)/\(params.filename)").path
